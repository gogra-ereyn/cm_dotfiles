...
local function render_cut(input, outpath, start, duration, input_mtime, exact)
    local args = {
        "-ss",
        tostring(start),
        "-t",
    ...
    if exact then
        ...
        table.insert(args, "medium")
        table.insert(args, "-crf")
        table.insert(args, "0")
    elseif options.lossless then
        table.insert(args, "-c")
        table.insert(args, "copy")
    end
...
local function cut_render(exact)
    ...
    for i, cut in ipairs(cuts) do
        if cut.end_time then
            ...
            local success = render_cut(
                input,
                cut_path,
                cut.start_time - cache_offset,
                duration,
                mtime,
                exact
            )
...
local function cut_render_copy()
	cut_render(false)
end

local function cut_render_exact()
	cut_render(true)
end
...
mp.add_key_binding("r", "cut_render", cut_render_copy)
mp.add_key_binding(nil, "cut_render_exact", cut_render_exact)
